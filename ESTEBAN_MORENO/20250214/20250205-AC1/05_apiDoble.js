...
miAPI.get('/paises', (req, res) => urbano.damePaises()
    .then (paises => {
            console.log('Piden lista de paises para Europa y devuelvo', paises.length,'elementos');
            res.json(paises);
        }
    ));
...
miAPI.get('/filtro', async (req, res) => {
    let campos=req.query;
    console.log('Piden ciudades:', campos);
    let ciudades= await urbano.dameCiudadesFiltro( campos.Caracter, campos.CountryCode);
    console.log('Piden lista de ciudades y devuelvo', ciudades.length,'elementos');
    res.json(ciudades);
});

miAPI.get('/paises/:continente', async (req, res) => {
    let continente=req.params.continente;
    console.log('Piden Paises para el siguiente continente:', continente);
    // let [paises]= await urbano.damePaisesContinente(continente);
    let paises= await urbano.damePaisesV2(continente);
    // NC: console.log('Piden lista de Paises x Continente y devuelvo', paises.length,'elementos');
    // NC: res.json(paises);
    if (paises)
        res.status(200).json(paises);
    else
        res.status(404).send('No existen paises en el continente ', +continente);
});
...
miAPI.get('/civitas/:id', async (req, res) => {
    const codigo=req.params.id;
    console.log('Piden Paises para el continente:', codigo);
    // Buscamos el elemento por su codigo y obtenemos una referencia
    let candidato=await urbano.dameCiudad(codigo);
    if (candidato)
        res.status(202).json(candidato);    
...
miAPI.get('/civitas/:id', async (req, res) => {
    const codigo=req.params.id;
    console.log('Piden filtradda por caracter', codigo);
    // Buscamos el elemento por su codigo y obtenemos una referencia
    let candidato=await urbano.dameCiudadFiltro(codigo);
    if (candidato)
        res.status(202).json(candidato);    
    else
        res.status(404).send('No existe ciudad que comiencen por '+codigo);
});
