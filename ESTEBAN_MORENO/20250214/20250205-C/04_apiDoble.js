...
import fauna from './04_animFunc.js';
...
miAPI.get('/civitas/:id', async (req, res) => {
    ...
    let listaCandidato=await urbano.dameCiudad(codigo);
    if (listaCandidato.length>0)
        res.status(302).json(listaCandidato);    
    else
...
miAPI.delete('/civitas/:id', (req, res) => {
    const codigo=req.params.id;
    console.log('Quieren borrar el ciudad con código', codigo);
    if (urbano.eliminaCiudad(codigo))
...
miAPI.get('/datos/:id', (req, res) => {
    ...
    // Buscamos el elemento por su codigo y obtenemos una referencia
    let candidato=fauna.dameAnimal(codigo);
    if (candidato)
