...
async function dameCiudades() {
    const filtro=['N','ESP'];
    const sentenciaSQL="SELECT id,Name, District, Population FROM city WHERE LEFT(Name,1)=? AND CountryCode=?;";
...
async function modificaCiudad(candidato, campos) {
    let cambiado=false;
    let sentenciaSQL = "Update FROM city WHERE ID=?;";
    console.log('Modificando Ciudad:', codigo);
    // Comprueba si debe cambiar nombre
    if ('Name' in campos && campos.Name!=candidato.Name) {
        candidato.Name=campos.Name;
        cambiado=true;
    }
    //Comprueba si debe cambiar distrito
    if ('District' in campos && campos.District!=candidato.District) {
        candidato.District=campos.District;
        cambiado=true;
    }
    // Comprueba si debe cambiar patas
    if ('Population' in campos && campos.Population!=candidato.Population) {
        candidato.Population=parseInt(campos.Population); // Convierte a número !!!
        cambiado=true;
    }
    return cambiado;
}
