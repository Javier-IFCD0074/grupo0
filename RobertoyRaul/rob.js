...
document.getElementById("mostrar").addEventListener("click", filaUsario , true);
...
function filaUsario(persona, url_imagen) {
    let edad = calcularEdad(persona.dob);
    let mi_fila = '<tr>';
    mi_fila += `<td>${persona.name.title}</td>`;
    mi_fila += `<td>${persona.name.first} ${persona.name.last}</td>`;
    mi_fila += `<td>${persona.email}</td>`;
    mi_fila += `<td>${edad}</td>`;
    mi_fila += `<td><img src="${url_imagen}" alt="Foto del usuario"></td>`;
    mi_fila += '</tr>';
    return mi_fila;
}

function calcularEdad(dob) {
    let edad = new Date().getFullYear() - dob.year;
    let mes = dob.month;
    if (new Date().getMonth() < mes || 
        (new Date().getMonth() === mes && new Date().getDate() < dob.day)) {
        edad--;
    }
    return edad;
}
...
function filtrarUsuarios(numUsuarios) {
    fetch("https://randomuser.me/api?results=" + numUsuarios)
        .then(res => res.json())
        .then(data => {
            contenido.innerHTML = '';
            let mi_tabla = '<table border=1><thead><tr><th>Title</th><th>Nombre</th><th>Apellido</th><th>Email</th><th>Edad</th><th>Foto</th></tr></thead><tbody>';
